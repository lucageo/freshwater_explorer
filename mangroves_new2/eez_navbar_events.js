$(document).ready(function () {
    ...
    function show_hide_collapsible(target, main_li, info_type) {
        ...
        if (target.hasClass('info')) {
            if (target.hasClass('on') == false) {
                ...
                $('#slide-out').animate({
                    //  scrollTop: main_li.find(".row").eq(1).offset().top
                    scrollTop: main_li.offset().top + 50
                }, 2000);
    ...
    $('.sidenav .main_li .collapsible-body .list').on('click', function (e) {
        var target = $(e.target);
        if (typeof target == 'undefined')
            return false;

        console.log(console.warn(target.hasClass('title') && target.hasClass('show')))
        console.info(target)
        var container = target.parents().closest('.summary');
        ...
        if (target.hasClass('material-icons')) {
            ...
            show_hide_collapsible(target, main_li, info_type)

        }
        else {

            console.warn(target.hasClass('title') && target.hasClass('show'))

            if (target.hasClass('title') && target.hasClass('show')) {
                // if (target.attr('class') == 'title show') {
                var target = container.find('.info')
                show_hide_collapsible(target, main_li, info_type)
            }
        }

    })
    $('.sidenav .show_hide_table').click(function () {
